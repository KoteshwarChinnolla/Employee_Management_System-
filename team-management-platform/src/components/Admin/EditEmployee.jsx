...
const EditEmployee = () => {
    ...
    useEffect(() => {
        if (name) {
            fetch(`${API_BASE_URLS.EMPLOYEE}/employee/getEmployeeByName/${encodeURIComponent(name.replace(/ /g,'-'))}`)
                .then(res => res.json())
                .then(data => {
                    setForm({
                        id: data.id,
    ...
    if (loading) return <div style={{textAlign: 'center', marginTop: 80}}>Loading employee details...</div>;
