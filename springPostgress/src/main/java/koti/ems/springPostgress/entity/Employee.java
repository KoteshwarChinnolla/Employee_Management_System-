...
import koti.ems.springPostgress.dto.ContactDto;
...
public class Employee {
    ...
    // // 2025-06-01T01:12:13.564+05:30  WARN 17744 --- [springPostgress] [nio-8080-exec-3] .c.j.MappingJackson2HttpMessageConverter : Failed to evaluate Jackson deserialization for type [[simple type, class koti.ems.springPostgress.entity.Employee]]: com.fasterxml.jackson.databind.exc.InvalidDefinitionException: Cannot handle managed/back reference 'defaultReference': no back reference property found from type `java.util.List<koti.ems.springPostgress.entity.Achivements>`
    ...
    public List<ProjectDto> getProjects(){
        ...
        EmployeeTeam employeeTeam = CurrentEmployeeTeam;
        if(employeeTeam  != null) {
            return employeeTeam.getAllProjects().stream()
    ...
    public List<Projects> getProjectsList(){
        ...
        for (EmployeeTeam team : employeeTeams) {
            projectsList.addAll(team.getAllProjects());
        }
        return projectsList;
    }

    public Projects getProjectsDetail(){
        EmployeeTeam CurrentEmployeeTeam = employeeTeams.size() > 0 ? employeeTeams.get(0) : null;
        if(CurrentEmployeeTeam  != null) {
            return CurrentEmployeeTeam.getCurrentProject();
        }
        return null;
    }
    ...
    public TeamDto getTeamByDto(){
            if(employeeTeams != null && !employeeTeams.isEmpty()) {
            return employeeTeams.get(0).getTeamDto();
        }
        return null;
    }
